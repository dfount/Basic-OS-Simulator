...
void Process::updateState(State newState, int currentTime) {
    // If the process is moving to READY or WAITING, increment waiting time
    if ((newState == State::READY || newState == State::WAITING) && currentTime > arrival_time) {
        waiting_time++;  // Increase waiting time by 1 unit (or more depending on your simulation timing)
    }

    // If the process is moving to TERMINATED, calculate turnaround time
    if (newState == State::TERMINATED && turnaround_time == 0) {
        turnaround_time = currentTime + waiting_time - arrival_time; // Total time since arrival
    }
